...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            idxInorder[inorder[i]] = i; // 노드의 번호에 해당하는 idxInorder 인덱스에 .inorder인덱스 저장
        }
    ...
    static void findPreorder(int inStart, int inEnd, int postStart, int postEnd) {
        if(inStart > inEnd || postStart > postEnd ) return; // base case

        int root = postorder[postEnd]; // postorder에서 마지막 값이 현재의 root
